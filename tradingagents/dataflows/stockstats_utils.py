...
def _fetch_ohlcv(symbol: str, canonical: str, start_str: str, end_str: str) -> pd.DataFrame:
    """Download OHLCV honoring the configured ``technical_indicators`` vendor chain.

    yfinance remains the historical default. When the config names ``akshare``
    (or the chain includes it and yfinance is unavailable — e.g. Yahoo blocked
    from a mainland-China network), the akshare/Sina downloader is used instead.
    Returns a frame carrying a ``Date`` column; raises ``NoMarketDataError``
    when no configured vendor returns usable data.
    """
    from .config import get_config
    ...
    if not vendors:
        vendors = ["yfinance", "akshare"]  # default chain: every available vendor
    ...
    for vendor in vendors:
        if vendor == "yfinance":
            try:
                ...
                # An empty yfinance frame is ambiguous: it can be a genuine
                # unknown symbol, OR Yahoo rate-limiting (HTTP 429) that returns
                # an empty download instead of raising. Don't abort the chain —
                # give the next configured vendor (e.g. akshare) a chance.
                logger.warning("yfinance returned no rows for %s: %s", canonical, exc)
                errors.append(f"yfinance: {exc}")
                continue
            except Exception as exc:  # noqa: BLE001 — try the next vendor in the chain
                logger.warning("yfinance OHLCV fetch failed for %s: %s", canonical, exc)
                errors.append(f"yfinance: {exc}")
                continue
        elif vendor == "akshare":
            try:
                ...
                # The akshare helper filters to its own 5y window; pass today so
                # the caller's curr_date filter below does the real look-ahead cut.
                ...
                # Same reasoning as yfinance: let the next configured vendor try.
                logger.warning("akshare returned no rows for %s: %s", canonical, exc)
                errors.append(f"akshare: {exc}")
                continue
            except Exception as exc:  # noqa: BLE001 — try the next vendor in the chain
                logger.warning("akshare OHLCV fetch failed for %s: %s", canonical, exc)
                errors.append(f"akshare: {exc}")
                continue

    detail = "; ".join(errors) if errors else "no configured vendor available"
...
def load_ohlcv(symbol: str, curr_date: str) -> pd.DataFrame:
    ...
    if data is None:
        ...
        if downloaded.empty or "Close" not in downloaded.columns:
            raise NoMarketDataError(
                symbol, canonical, "no vendor returned usable rows"
            )
